...
namespace Gtr {
	...
	struct AxisConfig
	{
		...
		unsigned long absoluteEncID;// <立即生效>		电机处于串口通信上的链路ID
	...
	struct GroupConfig
	{
		...
		short nOriPrfMode;				//  <复位生效>	姿态规划模式[0,3]：0劣弧， 1优弧，2固定（起点）,  3 跟随。默认0
		short nOriPassingMidPoint;		//  <复位生效>	圆弧插补辅助点（中间点）姿态是否生效[0,1]：0无效，1有效。默认0

		short nMotionDir[8];			// <立即生效> 插补运动的旋转轴运动方向。
													// 0:正方向
													// 1:负方向
													// 2:当前运动方向
													// 3:最短路径方向
		...
		//MOVS默认运动参数
		double movsVel;				// <立即>	MOVS默认速度，取值范围：(0,...)，单位：mm/s。
		double movsAcc;				// <立即>	MOVS默认加速度，取值范围：(0,...)，单位：mm/s^2。
		double movsDec;				// <立即>	MOVS默认减速度，取值范围：(0,...)，单位：mm/s^2。
		...
		double acsVelLimit[8];        // <复位生效>    五轴串联机器人ACS速度限制，单位：mm/s或度/s。
	...
	struct ControllerConfig
	{
		...
		//short gxnExtDiNum;		// <重启生效>	GXN扩展DI数量
		//short gxnExtDoNum;		// <重启生效>	GXN扩展DO数量

		short localDiNum;			//本地DI数量
		short localDoNum;			//本地DO数量
		...
		char sGxnExtModuleType1[16];	//<重启> 扩展模块1的型号
		char sGxnExtModuleType2[16];	//<重启> 扩展模块2的型号
		char sGxnExtModuleType3[16];	//<重启> 扩展模块3的型号
		char sGxnExtModuleType4[16];	//<重启> 扩展模块4的型号
		char sGxnExtModuleType5[16];	//<重启> 扩展模块5的型号
		char sGxnExtModuleType6[16];	//<重启> 扩展模块6的型号
		short sSaveLogToFile;			//<立即> 系统掉电日志保存 0:退出系统时保存日志 1:退出系统不时保存日志
		short hmiSubMemuType;			//<重启> 系统HMI子菜单风格, 0=列表式子菜单，1=平铺式子菜单

		bool bEthercatEnable;  // <重启>  是否启用ETHERCAT功能
		double nEthercatOverTime;  // <重启> ethercat开卡超时时间，单位:s(秒)
	};
	...
	struct ChannelConfig
	{
		...
		double shiftDeltaValue[3];								//立即生效 SHIFTON、SHIFTOFF指令X、Y、Z方向插补距离，单位mm

		char forcedReplayStartPrg[256];		//<立即> 回放模式强制启动程序目录
	};
	...
	struct CarrierConfig {
		short carrierMode;		//传送带类型：0-连续传送带，1-间歇传送带

		bool encReverse;		//编码器方向取反
		ENCODER_TYPE encType;   //编码器类型
		short encIndex;    		//编码器索引索引
		short remoteEncStationId;//远程编码器站号
		double encScale;		//脉冲当量，单位pulse/ms

		double detectPos;     	//传送带检测位相对传送带坐标系零点的X轴偏移量,单位ms
		double parkPos;       	//传送带泊车位相对传送带坐标系零点的X轴偏移量,单位ms
		double stopPos;       	//传送带停止位相对传送带坐标系零点的X轴偏移量,单位ms
		double syncDelay;         	//同步区任务执行前的延时时间,单位ms
		double sampleTime;    	//传送带速度采样时间,单位ms
		double velMax;        	//机器人最大追赶速度,单位mm/s
		double acc;           	//机器人追赶加速度,单位mm/s^2
		double cbOrigin[6];   	//传送带坐标系零点相对机器人MCS的偏移和旋转
		short robotGroup;       //工作机器人group号
		short filterNum;		//编码器滤波时间
	};

	struct CarrierObject {
		short objectId;			//工件对象ID
		double encPos;			//工件对象经过检测位时的编码器位置，单位mm
		double pose[6];         //工件对象相对传送带坐标系的位置和姿态
		double processTime;     //工件对象在同步区的最大处理时间，单位ms
	};

	struct CarrierStatus {
		bool running;                   // 是否已启动传送带跟踪
		...
		short objectId;                  // 当前正在处理的工件标识
		short empty;					// 工件缓冲区是否为空

		unsigned long receiveCount;     // 传送带接受到的所有工件数量
		unsigned long RemainderCount;   // 传送带剩余还未处理的工件数量
		unsigned long processCount;     // 传送带已成功处理的工件数量，
		unsigned long discardCount;		//传送带丢弃的工件数量

		double robotStartPos;			// 传送带X轴方向，机器人启动位置相对传送带坐标系的位置
		double objectPos;				// 传送带X轴方向，实际工件相对传送带坐标系的位置
		double robotPos;				// 传送带X轴方向，机器人相对工件的位置
	};
	...
	{
		bool is_starting;				///<正在开始监听
		bool is_started;				///<已经监听中
		bool is_err;					///<出现错误
	};
	...
	struct ModbusTcpClientStatus
	{
		bool is_connecting;				///<正在连接状态
		bool is_connected;				///<已经连接上
		bool is_err;					///<出现错误

		short last_error_code;
	};

	struct ModbusTcpServerInfo
	{
		bool b_enable;					///<使能
		char ip[20];					///<IP地址
		unsigned short port;			///<端口
	...
	struct ModbusTcpClientInfo
	{
		bool b_enable;					///<通信启用使能
		bool b_read;					///<通信周期读使能
		bool b_write;					///<通信周期写使能
		char ip[20];					///<IP地址
		unsigned short port;			///<端口
	...
	struct TRGroupPosTransformInput
	{
		short coordSystem;
		short oriMode;
		short configIndex;
	...
	struct TRGroupPosTransformOutput
	{
		short configIndex;
		short singularity;
		short reserve1[10];
	...
	struct GroupSuperpositionTable
	{
	...
	struct GroupSuperpositionTablePrm
	{
	...
	struct GroupSuperpositionTableData
	{
		double value[8];
	};

	struct GroupSuperpositionTableStatus
	{
		short workIndex;
		short reserve1[7];
		double reserve2[14];
	...
	struct StAbsEncPos
	{
		short stationId;				//从站的驱动器在等环网中的站号
		short axis;						//轴ID，从1开始索引
		unsigned long long multiLines;	//编码器多圈数
	};

	struct GroupMoveSegmentInfo
	{
		long userTag;
		long segNum;
	...
	* @return		返回输出数据的长度
	*/
	typedef short(*GTRobotTcpServerOnMessageCallback)(HANDLE, HANDLE);

	/**
	* @brief		TCP客户端。接收到新数据的回调函数指针原型
	* @param[in] 	当前客户端的句柄
	* @param[out]	无
	* @note			无
	* @return		返回输出数据的长度
	*/
	typedef short(*GTRobotTcpClientOnMessageCallback)(HANDLE);
}
